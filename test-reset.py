...
def check_execv_support():
    """
    os.execv qo'llab-quvvatlanishini sinaydi.
    Misol uchun, sinov uchun kichik dastur ishga tushuradi va keyin to'xtatadi.
    """
    try:
        # Qisqa test sifatida o'zi o'zini ishga tushirishga harakat qilamiz,
        # lekin darhol to'xtatamiz.
        # Bu haqiqiy execv sinovi emas, ammo ko'pchilik muhitlarda ishlaydi.
        # Shunchaki try except orqali xatoni tekshiramiz.
        python = sys.executable
        # Bu yerda subprocessda boshqa skript ishga tushmaydi,
        # shuning uchun sinov to'liq bo'lmasligi mumkin.
        # Ammo execv os da bo'lishi kerak.
        os.execv(python, [python, "-c", "print('test')"])
    except Exception as e:
        print("os.execv ishlamayapti:", e)
        return False
    # Agar execv ishlasa, bizga bu yerga kelmaydi,
    # chunki execv jarayonni to'xtatadi, shuning uchun True qaytarish uchun
    # testni boshqa usulda qilish kerak.
    return True

def check_subprocess_support():
    """
    subprocess ishlashini sinaydi.
    """
    try:
        p = subprocess.Popen([sys.executable, "-c", "print('hello')"])
        p.wait(timeout=5)
