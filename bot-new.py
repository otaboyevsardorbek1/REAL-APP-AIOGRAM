...
#===local==settings
LOG_FILE = "bot.log"
BOT_TOKEN = "7552988744:AAHZYsxPA_QZvvRGY0Jch-4cCEDI0meuR5E"
MAX_LOG_SIZE_MB=50
...
OWNER_ID = [6646928202,6130650407]  # Owner ID
...
async def ERROR_TO_ADMIN_SEND(update: types.Update, exception: Exception):
    try:
       for chat_id in OWNER_ID:
        ...
        loggeradmin.error(f"Xato yuborishda muammo: {e}")
...
def check_user_level_create(user: User) -> str:
    if user.role == "owner":
...
COMMANDS_BY_ROLE = {
    "owner": [
        ...
        BotCommand(command="logs", description="📝 Log fayllarini ko‘rish"),
...
async def cmd_set_role(message: types.Message, bot: Bot):
    ...
    async with async_session_maker() as session:
        ...
        # Endi current_user mavjud, undan foydalanish xavfsiz
        ...
        # Target foydalanuvchini olish yoki yaratish
        ...
        if not target_user:
            target_user = User(
                id=target_user_id,
                full_name=f"{target_user_id} <--> {history} <--> Qo‘shildi!",
                role=new_role
            )
    ...
    await message.answer(f"✅ Foydalanuvchi {target_user_id} roli {new_role} ga o‘zgartirildi.")
...
async def get_user_profile_photo(message: Message):
    ...
    caption = (
        ...
        f"🌐 Til kodi: {user.language_code if user.language_code else 'Noma’lum'}\n"
