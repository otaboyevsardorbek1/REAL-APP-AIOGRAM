...
WEBHOOK_HOST = os.getenv("WEBHOOK_HOST", "https://2d9f12a6aef9.ngrok-free.app")
...
class MaintenanceMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: types.TelegramObject, data):
        ...
        # faqat Message eventlarida ishlaydi
        if not isinstance(event, Message):
            return await handler(event, data)

        user_id = event.from_user.id
        user_text = (event.text or "").strip()
        user_lang = event.from_user.language_code or "any"

        logger.info(f"[Middleware] User {user_id} sent message, lang: [{user_lang}]")

        # Agar texnik rejim yoqilgan bo‘lsa
        if MAINTENANCE_MODE:
            # admin va uning ruxsatli buyruqlari bundan mustasno
            if not (user_id in ADMIN_ID and user_text in admin_panel_commands):
                return await self._send_maintenance_message(event, user_lang)

        # Aks holda, davom etadi
        return await handler(event, data)

    async def _send_maintenance_message(self, event: Message, lang: str):
        """Har qanday xabar uchun texnik rejim haqida info yuboruvchi funksiya"""
        messages = {
            "uz": "🛠 Texnik ishlar olib borilmoqda. Iltimos, keyinroq urinib ko‘ring.",
            "ru": "🛠 Ведутся технические работы. Пожалуйста, попробуйте позже.",
            "en": "🛠 Maintenance work is underway. Please try again later.",
            "any": "🛠 Maintenance work is underway. Please try again later.",
        }
        msg = messages.get(lang, messages["any"])
...
async def settings_handler(message: Message):
    ...
    data=(
        "🧪 Sozlamalar:\n"
        "/set_webhook - webhook 1\n"
        "/delete_webhook - webhookni pollingga o‘tkazish\n"
        "/webhook_info - webhook info"
    )
    return await message.answer(text=str(data))
...
async def webhook_info_handler(message: Message, bot: Bot):
    if message.from_user.id in ADMIN_ID:
        info = await bot.get_webhook_info()
        # WebhookInfo obyektidagi maydonlar
        text = (
...
async def main():
    ...
    # Webhook o‘rnatilmagan bo‘lsa, polling boshlaymiz:
